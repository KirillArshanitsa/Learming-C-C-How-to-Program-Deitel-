...
void sortWords(char[][PHONE_NUMBER_SIZE]);
...
void sortWords(char allWords[][PHONE_NUMBER_SIZE])
{
	char tmpStr[PHONE_NUMBER_SIZE];
	unsigned short allCompareCount = TOTAL_RAND_GENERATION - 1;

	for (unsigned short e = 0; e < allCompareCount; e++) {
		for (unsigned short i = e + 1; i < TOTAL_RAND_GENERATION; i++) {
			if (allWords[i][0] != NULL) {
				if (strcmp(allWords[e], allWords[i]) > 0) {
					strcpy_s(tmpStr, allWords[e]);
					strcpy_s(allWords[e], allWords[i]);
					strcpy_s(allWords[i], tmpStr);
				}
			}
		}
	}
