...
void enterString(int[], unsigned int);
void printString(int[], unsigned int);
...
void enterString(int menory[], unsigned int operand)
{
    int strSize = 0;
    printf("Enter string size: ");
    scanf("%d", &menory[operand]);
    if(menory[operand] > 0) {
        strSize = menory[operand];
        printf("\nBegin enter string size %d:\n", strSize);
        char str[strSize + 1];
        scanf("%s", str);
        for (int i = 0 ; i < strSize; i++)
            menory[++operand] = str[i];
    }
    else{
        printf("You enter not valid string size %d, must be > 0\n", menory[operand]);
        return;
    }
}

void printString(int memory[], unsigned int operand)
{
    int strSize = 0;
    if (memory[operand] > 0) {
        strSize = memory[operand];
        printf("Print string size %d:\n", strSize);
        ...
        printf("String size = %d, must be > 0\n", memory[operand]);
        return;
    }
}
